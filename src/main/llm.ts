 ...
 * emailInstruction as the tone/style guide. Falls back with an error if
 * Ollama is not reachable — caller can use the static template instead.
 */
export async function draftEmail(input: ApplicationInput): Promise<EmailDraft> {
    ...
    const prompt = `Du entwirfst eine Bewerbungs-E-Mail für einen deutschen Bewerber.
